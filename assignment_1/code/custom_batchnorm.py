...
class CustomBatchNormAutograd(nn.Module):
    ...
    def forward(self, input):
        ...
        if input.shape[1] != self.n_neurons:
            raise ValueError('dimension of the input tensor {} does not match with channel size of the layer {}'.format(input.shape, self.n_neurons))

        mus = input.mean(dim=0)
        variances = ((input - mus)**2).mean(dim=0)
        input_norm = (input - mus) / (variances + self.eps)**0.5
...
class CustomBatchNormManualFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, gamma, beta, eps=1e-5):
        ...
        mus = input.mean(dim=0)
        variances = ((input - mus)**2).mean(dim=0)
        input_norm = (input - mus) / (variances + eps)**0.5
        out = gamma * input_norm + beta
        ctx.save_for_backward(input_norm, gamma, beta)
        ctx.eps = eps
        ctx.denom = torch.sqrt(variances + eps)
        ########################
        # PUT YOUR CODE HERE  #
        #######################
    ...
    def backward(ctx, grad_output):
        ...
        # PUT YOUR CODE HERE  #
        #######################
        input_norm, gamma, beta = ctx.saved_tensors
        input_norm_flag, gamma_flag, beta_flag = ctx.needs_input_grad
        grad_input_norm = grad_output * gamma
        if gamma_flag:
            grad_gamma = torch.sum(grad_output * input_norm, dim=0)
        if beta_flag:
            grad_beta = torch.sum(grad_output, dim=0)
        if input_norm_flag:
            grad_input = 1 / ctx.denom * 1 / input_norm.shape[0] * \
                (input_norm.shape[0] * grad_input_norm - torch.sum(grad_input_norm, dim=0) - input_norm * torch.sum(grad_input_norm * input_norm, dim=0))
...
class CustomBatchNormManualModule(nn.Module):
    ...
    def forward(self, input):
        ...
        if input.shape[1] != self.n_neurons:
            raise ValueError('dimension of the input tensor {} does not match with channel size of the layer {}'.format(input.shape, self.n_neurons))
        batchnorm = CustomBatchNormManualFunction()
        out = batchnorm.apply(input, self.gamma, self.beta, self.eps)
        # raise NotImplementedError
        ########################
        # END OF YOUR CODE    #
        #######################
